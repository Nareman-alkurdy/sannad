...
const StatsSection = () => {
  ...
  const stats = [
    { icon: <HomeIcon fontSize="large" />, value: "10.5k", label: "Sallers active our site" },
    { icon: <AttachMoneyIcon fontSize="large" />, value: "33k", label: "Monthly Product Sale" },
    { icon: <PersonIcon fontSize="large" />, value: "45.5k", label: "Customer active in our site" },
    { icon: <WorkOutlineIcon fontSize="large" />, value: "25k", label: "Annual gross sale in our site" },
  ];
      ...
      {stats.map((stat, idx) => (
        <div
          ...
          className={`cursor-pointer flex flex-col items-center p-6 border rounded-lg w-44 transition-colors duration-300 shadow-md ${
            activeIndex === idx ? "bg-red-500 text-white" : "bg-white text-black"
          }`}
        >
          {/* دائرة رملية خارجية */}
